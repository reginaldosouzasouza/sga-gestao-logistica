...
$(document).ready(function() {
    ...
    function atualizarIndices() {
        $('.item').each(function(index) {
            $(this).find('select, input').each(function() {
                ...
                if (name) {
                    name = name.replace(/\[\d+\]/, '[' + index + ']'); // Atualiza o índice no nome
                    $(this).attr('name', name);
                }
            });
        });
    ...
    $('#add-item').click(function() {
        ...
        atualizarIndices(); // Atualiza os índices
    });
    ...
    $(document).on('click', '.remove-item', function() {
        ...
        atualizarIndices(); // Reorganiza os índices
    });



// Atualizar os valores totais do item ao alterar a quantidade ou escolher um produto
$(document).on('change', '.produto-select, .quantidade', function () {
    ...
    function calcularTotais() {
        let totalPedido = 0;
        $('.item').each(function () {
            let totalItem = parseFloat($(this).find('.valor-total').val()) || 0;
            totalPedido += totalItem;
        });
        $('#total-pedido').text(totalPedido.toFixed(2));
    }
