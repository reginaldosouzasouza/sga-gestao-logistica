...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .linha-label .formula-explicacao {
            color: #64748b;
            font-size: 12px;
            line-height: 1.35;
        }
...
@php
    ...
    $totalCustosFuturos = (float) ($cards['total_custos_futuros'] ?? ($contasPagarAberto + $reposicaoEstimada));

    $custoMedioDiarioFuturo = (float) ($cards['custo_medio_diario_futuro'] ?? (
        $diasRestantes > 0 ? round($totalCustosFuturos / $diasRestantes, 2) : 0
    ));

    $sobraMediaDiariaProjetada = (float) ($cards['sobra_media_diaria_projetada'] ?? round(
        $mediaDiariaRecebimento - $custoMedioDiarioFuturo,
        2
    ));

    $previsaoRealFechamento = (float) ($cards['previsao_real_fechamento'] ?? round(
        $sobraMediaDiariaProjetada * $diasRestantes,
        2
    ));
...
<div class="container">
    ...
    <div class="grid-main">
        <div class="panel">
            <h2>Previsão Real de Fechamento com Reposição</h2>
            <p class="sub">Análise baseada no que ainda falta pagar, repor e receber</p>
            ...
            <div class="linha-analise">
                <span>Custo estimado de reposição</span>
                <strong class="text-orange">{{ moeda($custoSaldoReposicaoAComprar) }}</strong>
            <!--    <strong class="text-orange">{{ moeda($reposicaoEstimada) }}</strong>-->
            </div>

            <div class="linha-analise">
                <span>Total de custos futuros</span>
                <strong class="text-red">{{ moeda($totalCustosFuturos) }}</strong>
            </div>
            ...
            <div class="linha-analise tooltip-card"
                 data-tooltip="Cálculo: Total de custos futuros dividido pelos dias restantes. Fórmula: {{ moeda($totalCustosFuturos) }} ÷ {{ $diasRestantes }} dias = {{ moeda($custoMedioDiarioFuturo) }} por dia.">
                <div class="linha-label">
                    <span>Custo médio diário futuro</span>
                    <small class="formula-explicacao">
                        Total de custos futuros {{ moeda($totalCustosFuturos) }} ÷ {{ $diasRestantes }} dias restantes.
                    </small>
                </div>
                <strong class="text-orange">{{ moeda($custoMedioDiarioFuturo) }}</strong>
            </div>

            <div class="linha-analise tooltip-card"
                 data-tooltip="Cálculo: Média diária de recebimento até ontem menos o custo médio diário futuro. Fórmula: {{ moeda($mediaDiariaRecebimento) }} - {{ moeda($custoMedioDiarioFuturo) }} = {{ moeda($sobraMediaDiariaProjetada) }} por dia.">
                <div class="linha-label">
                    <span>Sobra média diária projetada</span>
                    <small class="formula-explicacao">
                        Média diária recebida {{ moeda($mediaDiariaRecebimento) }} - custo médio diário futuro {{ moeda($custoMedioDiarioFuturo) }}.
                    </small>
                </div>
                <strong class="{{ $sobraMediaDiariaProjetada >= 0 ? 'text-green' : 'text-red' }}">
                    {{ moeda($sobraMediaDiariaProjetada) }}
                </strong>
            </div>

            <div class="linha-analise tooltip-card"
                 data-tooltip="Cálculo: Sobra média diária projetada multiplicada pelos dias restantes. Fórmula: {{ moeda($sobraMediaDiariaProjetada) }} × {{ $diasRestantes }} dias = {{ moeda($previsaoRealFechamento) }}.">
                <div class="linha-label">
                    <span>Previsão real de fechamento</span>
                    <small class="formula-explicacao">
                        Sobra média diária projetada {{ moeda($sobraMediaDiariaProjetada) }} × {{ $diasRestantes }} dias restantes.
                    </small>
                </div>
                <strong class="{{ $previsaoRealFechamento >= 0 ? 'text-green' : 'text-red' }}">
                    {{ moeda($previsaoRealFechamento) }}
                </strong>
            </div>

            <div class="decisao {{ $previsaoRealFechamento >= 0 ? 'status-ok' : 'status-risk' }}">
                <strong>
                    {{ $previsaoRealFechamento >= 0 ? 'Sobra projetada no fechamento' : 'Falta projetada no fechamento' }}
                </strong>

                @if($previsaoRealFechamento >= 0)
                    Mantendo a média diária de recebimento até ontem, o período tende a fechar positivo mesmo após considerar as contas em aberto e a reposição estimada.
                @else
                    Mantendo a média diária de recebimento até ontem, o período tende a fechar negativo após considerar as contas em aberto e a reposição estimada.
                @endif
            </div>
        </div>
